...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, onNavigate, onAddToCart, cartQuantity, isFavorite, onToggleFavorite, backScreen }) => {
  ...
  const currentPrice = selectedSize === 'large' ? product.price + 20 : product.price;
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      message.textContent = '已加入购物车';
      message.className = 'fixed top-20 left-1/2 -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-full shadow-lg z-50 animate-bounce';
      ...
      <div className="sticky bottom-0 left-0 right-0 p-5 bg-white dark:bg-surface-dark border-t border-gray-100 dark:border-gray-700/50 rounded-t-3xl shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-20 pb-safe">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            disabled={isAdding}
            className="flex-1 h-12 bg-primary hover:bg-primary-dark text-white rounded-xl font-bold text-base flex items-center justify-center gap-2 shadow-lg shadow-primary/30 active:scale-95 transition-all disabled:opacity-50"
          >
            <span className="material-symbols-outlined">add</span>
            {isAdding ? '添加中...' : `加入购物车 - ${selectedSize === 'standard' ? '标准' : '大份'}`}
          </button>
        </div>
      </div>
    </div>
